...
def device_put(value, sharding):
  # Handle both dict of values and single sharding, or structured shardings
  def put(x, s):
    # For scalars (rank 0), use empty PartitionSpec
    if np.ndim(x) == 0:
      scalar_sharding = jax.sharding.NamedSharding(s.mesh, jax.sharding.PartitionSpec())
  ...
  if is_multihost():
    ...
    # Check if sharding is a dict (structured) or single sharding
    if isinstance(sharding, dict):
      # Both value and sharding are dicts with same structure
      value = jax.tree.map(lambda x, s: put(x, s), value, sharding, is_leaf=lambda x: isinstance(x, np.ndarray))
    else:
      # Single sharding applied to all leaves
      value = jax.tree.map(lambda x: put(x, sharding), value, is_leaf=lambda x: isinstance(x, np.ndarray))
  return value
